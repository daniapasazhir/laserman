...
template <GS_driverType _DRV, uint8_t _AXLES, OnNext onNext, uint8_t _BUF = 32>
class GPlanner2 {
   public:
    GPlanner2() {
        setAcceleration(100);
    ...
    bool tickManual() {
        ...
        if (shift)
            if (++substep & ((1 << shift) - 1)) return status > 1;  // пропускаем сабшаги
        step++;
    ...
    void addTarget(int32_t tar[], uint8_t l, GS_posType type = ABSOLUTE) {
        if (type == ABSOLUTE)
            ...
            for (int i = 0; i < _AXLES; i++) bufP[i].add(tar[i] + bufP[i].getLast());
        bufL.add(l);
        bufV.add(0);
        bufS.add(0);
    }
   ...
   private:
    ...
    bool setTarget() {
        ...
        for (uint8_t i = 0; i < _AXLES; i++) {                        // для всех осей
            ...
            if (steppers[i]->pos != bufP[i].get(1)) {                       //__MODIFIED__
